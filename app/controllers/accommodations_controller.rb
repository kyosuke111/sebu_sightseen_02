class AccommodationsController < ApplicationController

  def index
    @targets = Accommodation.all.order(name_ja: "ASC")
    @num_targets = @targets.count
    @list_categorys = Accommodation.select(:category_ja).distinct.order(category_ja: "ASC")
    @list_areas = Accommodation.select(:area_ja).distinct.order(area_ja: "ASC")
    @q = Accommodation.ransack(params[:q])
    @search_results = @q.result(distinct: true)
    gon.target_datas = @targets
  end
  
  def search
    @list_categorys = Accommodation.select(:category_ja).distinct.order(category_ja: "ASC")
    @list_areas = Accommodation.select(:area_ja).distinct.order(area_ja: "ASC")
    @q = Accommodation.search(search_params)
    @search_results = @q.result(distinct: true).order(name_ja: "ASC")
    @num_targets = @search_results.count
  end

  def show
    @target = Accommodation.find(params[:id])
    gon.target_datas = @target
  end

  private
  def search_params
    params.require(:q).permit!
  end


end
